...
- `zurtr test` maps to module-scoped Zig test steps; only the modules named are
  ...
  `test-zeex-live` with `-Dzscript=true`. `zig build test` aggregates all of them,
  `test-cli` included; that dependency is worth reading against the steps that exist
  rather than trusted, because a missing one fails nothing — it just makes the run
  smaller, which is how it went missing once. `zeex-live` is a step of its own too,
  because it installs a tool rather than running tests.
