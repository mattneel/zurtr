...
|2026-09-23|`src/zgpu.zig:1816`|`@import("builtin").mode == .Debug` → `.debug`: 0.17 lowercased enum fields, so `Optimize` is `.debug` / `.release_fast`. **Not reachable from zgpu's own test suite** - it sits behind the device-lost handler, which tests cannot trigger. Found by building `examples/hello-gpu`, the first real consumer.|
|2026-09-23|`src/zgpu.zig` (`GetModuleHandleW`)|Second break reachable only by a consumer: **Zig 0.17 removed `GetModuleHandleW` from `std.os.windows.kernel32` entirely** (the file survives; the Win32 signatures are gone). Declared locally as `extern "kernel32" fn GetModuleHandleW(...)`. Found by cross-compiling `examples/hello-gpu` for `x86_64-windows-gnu`; the Linux path never reaches it.|
|2026-09-23|(unresolved)|Cross-compiling for Windows still fails at link: `lld-link: duplicate symbol: `std::__1::bad_function_call::~bad_function_call()`` (and its vtable). The Windows prebuilt Dawn archive carries its own libc++ typeinfo, and `zgpu/build.zig` sets `.link_libcpp = target.result.abi != .msvc`, so Zig links a second copy. **Known, not yet fixed** - the fix is either a linker relaxation, a different prebuilt, or the MSVC target, and none of them were worth guessing at.|
