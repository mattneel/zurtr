...
pub fn Pool(
    ...
    return struct {
        ...
        /// `Columns`' field names, types and attributes in parallel. Zig 0.17 replaced
        /// `std.meta.fields()` (which returned a slice of field descriptors) with this
        /// columnar form on `@typeInfo`.
        pub const column_fields = @typeInfo(Columns).@"struct";
        pub const column_count = column_fields.field_names.len;

        pub fn ColumnType(comptime column: Column) type {
            return column_fields.field_types[@backingInt(column)];
        }

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        const private_fields = @typeInfo(struct {
            @"Pool._free_queue": AddressableIndex,
            @"Pool._curr_cycle": AddressableCycle,
        }).@"struct";
        const private_field_count = private_fields.field_names.len;
        ...
        const Storage = blk: {
            ...
            var field_attrs: [field_count]std.builtin.Type.Struct.FieldAttributes = undefined;

            for (
                private_fields.field_names,
                private_fields.field_types,
                private_fields.field_attrs,
                field_names[0..private_field_count],
                field_types[0..private_field_count],
                field_attrs[0..private_field_count],
            ) |name, FieldType, attrs, *out_name, *out_type, *out_attrs| {
                out_name.* = name;
                out_type.* = FieldType;
                out_attrs.* = attrs;
            }
            ...
            for (
                ...
                column_fields.field_types,
                column_fields.field_attrs,
        ...
        pub fn getColumnPtrUnchecked(self: Self, handle: AddressableHandle, comptime column: Column) *ColumnType(column) {
            return &@field(self.columns, column_fields.field_names[@backingInt(column)])[handle.index];
        }
        ...
        pub fn getColumnsUnchecked(self: Self, handle: AddressableHandle) Columns {
            ...
            inline for (column_fields.field_names) |name| {
                @field(values, name) =
                    @field(self.columns, name)[handle.index];
            }
            return values;
        }
        ...
        pub fn setColumnUnchecked(self: Self, handle: AddressableHandle, comptime column: Column, value: ColumnType(column)) void {
            self.deinitColumnAt(handle.index, column);
            @field(self.columns, column_fields.field_names[@backingInt(column)])[handle.index] = value;
        }
        ...
        fn initColumnsAt(self: Self, index: AddressableIndex, values: Columns) void {
            inline for (column_fields.field_names) |name| {
                @field(self.columns, name)[index] =
                    @field(values, name);
            }
        }

        /// Call `value.deinit()` if defined.
        fn deinitColumnAt(self: Self, index: AddressableIndex, comptime column: Column) void {
            const ColumnT = ColumnType(column);
            switch (@typeInfo(ColumnT)) {
                .@"struct", .@"enum", .@"union", .@"opaque" => {
                    if (@hasDecl(ColumnT, "deinit")) {
                        @field(self.columns, column_fields.field_names[@backingInt(column)])[index].deinit();
                    }
                },
                else => {},
            }
        ...
        fn deinitColumnsAt(self: Self, index: AddressableIndex) void {
            if (@hasDecl(Columns, "deinit")) {
                ...
                inline for (0..column_count) |i| {
                    self.deinitColumnAt(index, @fromBackingInt(@intCast(i)));
                }
            }
        }
        ...
        fn updateSlices(self: *Self) void {
            ...
            inline for (column_fields.field_names, column_fields.field_types, 0..) |name, ColumnT, i| {
                const p = slice.ptrs[private_field_count + i];
                const f = @as([*]ColumnT, @ptrCast(@alignCast(p)));
                @field(self.columns, name) = f[0..slice.len];
            }
        }
...
const DeinitCounter = struct {
    ...
    // Zig 0.17's `@hasDecl` only sees public declarations, and the pool detects a column
    // type's `deinit` through it (as it must: a private one would not be callable from
    // here anyway), so the fixture's must be public.
    pub fn deinit(self: *Self) void {
        self.counter.* += 1;
    }
};
