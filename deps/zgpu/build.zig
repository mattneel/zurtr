...
pub fn build(b: *std.Build) void {
    ...
    // `@cImport` was removed in Zig 0.17: Dawn's C header is translated into a module
    // that `src/wgpu.zig`'s ABI compatibility test imports.
    const dawn_c = b.addTranslateC(.{
        .root_source_file = b.path("libs/dawn/include/dawn/webgpu.h"),
        .target = target,
        .optimize = optimize,
    });
    dawn_c.addIncludePath(b.path("libs/dawn/include"));
    const dawn_c_module = dawn_c.createModule();
    ...
    const tests = b.addTest(.{
        ...
        .root_module = b.createModule(.{
            ...
            .imports = &.{
                .{ .name = "zgpu_options", .module = options_module },
                .{ .name = "zpool", .module = b.dependency("zpool", .{}).module("root") },
                .{ .name = "dawn_c", .module = dawn_c_module },
            },
        }),
    });
