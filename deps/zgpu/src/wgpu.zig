...
test "extern struct ABI compatibility" {
    ...
    const wgpu = @import("dawn_c");
    inline for (comptime std.meta.declarations(@This())) |decl_name| {
        const ZigType = @field(@This(), decl_name);
            ...
            inline for (comptime std.meta.fieldNames(CType)) |c_field_name| {
                std.testing.expectEqual(
                    ...
                    std.log.err(
                        ...
                        .{ wgpu_name, c_field_name, decl_name, std.meta.fieldNames(ZigType)[i] },
                    );
                    return err;
                };
                i += 1;
