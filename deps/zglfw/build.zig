...
pub fn build(b: *std.Build) void {
    ...
    // `@cImport` was removed in Zig 0.17: GLFW's C header is translated into a module,
    // `glfw_c`, that the ABI compatibility tests in `src/zglfw.zig` import. `GLFW_INCLUDE_NONE`
    // keeps the header from pulling in `GL/gl.h` — none of the checked declarations need it.
    const glfw_c = b.addTranslateC(.{
        .root_source_file = b.path("libs/glfw/include/GLFW/glfw3.h"),
        .target = target,
        .optimize = optimize,
    });
    glfw_c.defineCMacro("GLFW_INCLUDE_NONE", "1");
    glfw_c.addIncludePath(b.path("libs/glfw/include"));
    ...
    const module = b.addModule("root", .{
        ...
        .imports = &.{
            ...
            .{ .name = "glfw_c", .module = glfw_c_module },
        },
    });
    ...
    tests.root_module.addImport("glfw_c", glfw_c_module);
