...
/// Copy a C `JSValue` into the binding's `Value`.
///
/// The two are the same extern struct by construction — `Value` mirrors `quickjs.h`'s layout field for
/// field — but Zig 0.17 refuses `@bitCast` between extern structs, so the bytes are copied instead.
/// The size assertion is the part worth having: if a future QuickJS changes the C layout, this fails
/// at compile time here rather than quietly reinterpreting values at runtime.
inline fn valueFromC(raw: c.JSValue) Value {
    comptime std.debug.assert(@sizeOf(Value) == @sizeOf(c.JSValue));

    var value: Value = undefined;
    @memcpy(std.mem.asBytes(&value), std.mem.asBytes(&raw));
